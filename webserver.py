        ...
        json_response["games"].append(
            {
                "game": {
                    ...
                    # This will hold all the characters of the game
                    "characters": []
                }
            }
        )
...
def newCharacter():
    ...
    if request.method == "POST":
        if request.form["name"] and request.form[
                "game"] and request.form["photo_url"]:
            game = session.query(Game).filter_by(
                name=request.form["game"]).one()
...
@app.route(
    "/games/<game_name>/characters/<character_name>/edit",
    methods=[
        "GET",
        "POST"])
@login_required
def editCharacter(character_name, game_name):
    ...
    editedCharacter = session.query(
        Character).filter_by(name=character_name).one()
    games = session.query(Game).all()
    # If the route was accessed with a POST method
    if request.method == "POST":
        ...
        if request.form["name"] and request.form["photo_url"] and game:
            ...
            flash(
                "Character Successfully Edited %s" %
                editedCharacter.photo_url)
...
@app.route(
    "/games/<game_name>/characters/<character_name>/delete",
    methods=[
        "GET",
        "POST"])
@login_required
def deleteCharacter(character_name, game_name):
    ...
    characterToDelete = session.query(
        Character).filter_by(name=character_name).one()
    # Require game instance to avoid "Parent instance is not bound
    # to a Session" error.
    ...
    if request.method == "POST":
        if characterToDelete:
            ...
            return redirect(
                url_for(
                    "showCharacters",
                    game_name=characterToDelete.game.name))
    else:
        return render_template(
            "deleteCharacter.html",
